...
class DoubleSqueezeDataset(WordLineDataset):
    ...
    def main_loader(self, subset, level) -> list:
        ...
        for bfile1 in box_files:
            if not bfile1[-12:] == '_letters.txt':
                print('Skipping nonstandard file name: '+bfile1)
                continue
            if 'Rotation1' in bfile1:
                bfile2 = bfile1.replace('Rotation1','Rotation2')
            else:
                # either rotation 2 or a non-actionable file
                continue
            if getBase(bfile1) in test_set:
                print('Skipping test set squeeze '+bfile1)
                continue
            if (subset == 'train') and getBase(bfile1) in valid_set:
                print('Reserving validation set squeeze '+bfile1)
                continue            
            if (subset != 'train') and not getBase(bfile1) in valid_set:
                continue             
            a1,b1,t1,l1 = readBoxFile(join(self.box_dir,bfile1))  
            a2,b2,t2,l2 = readBoxFile(join(self.box_dir,bfile2))  
            base1 = bfile1[:-12]
            base2 = bfile2[:-12]
            #base2pos = bfile.find('_Rotation')
            #base2 = bfile[:base2pos]
            if len(t1)!=len(t2):
                print('Transcript length mismatch in {}:  {} vs. {}'.format(bfile1,len(t1),len(t2)))
            for index in range(min(len(t1),len(t2))):
                ...
                if t1[index]!=t2[index]:
                    print('Transcript mismatch in {} line {}:  {} vs. {}'.format(bfile1,index,t1[index],t2[index]))
